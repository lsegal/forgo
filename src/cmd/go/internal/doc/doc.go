...
var CmdDoc = &base.Command{
	Run:         runDoc,
	UsageLine:   "forgo doc [doc flags] [package|[package.]symbol[.methodOrField]]",
	CustomFlags: true,
...
which is schematically one of these:
	...
	forgo doc <pkg>
	forgo doc <sym>[.<methodOrField>]
	forgo doc [<pkg>.]<sym>[.<methodOrField>]
	forgo doc [<pkg>.][<sym>.]<methodOrField>
...
directory is selected, so "forgo doc Foo" shows the documentation for symbol Foo in
the current package.

The package path must be either a qualified path or a proper suffix of a
path. The forgo tool's usual package mechanism does not apply: package path
elements like . and ... are not implemented by forgo doc.
...
Examples:
	...
	forgo doc Foo
		...
		a package path.)
	forgo doc encoding/json
		Show documentation for the encoding/json package.
	forgo doc json
		Shorthand for encoding/json.
	forgo doc json.Number (or forgo doc json.number)
		Show documentation and method summary for json.Number.
	forgo doc json.Number.Int64 (or forgo doc json.number.int64)
		Show documentation for json.Number's Int64 method.
	forgo doc cmd/doc
		Show package docs for the doc command.
	forgo doc -cmd cmd/doc
		Show package docs and exported symbols within the doc command.
	forgo doc template.new
		Show documentation for html/template's New function.
		(html/template is lexically before text/template)
	forgo doc text/template.new # One argument
		Show documentation for text/template's New function.
	forgo doc text/template new # Two arguments
		Show documentation for text/template's New function.

	At least in the current tree, these invocations all print the
	documentation for json.Decoder's Decode method:

	forgo doc json.Decoder.Decode
	forgo doc json.decoder.decode
	forgo doc json.decode
	cd go/src/encoding/json; forgo doc decode
...
func usage(flagSet *flag.FlagSet) {
	fmt.Fprintf(os.Stderr, "Usage of [forgo] doc:\n")
	fmt.Fprintf(os.Stderr, "\tforgo doc\n")
	fmt.Fprintf(os.Stderr, "\tforgo doc <pkg>\n")
	fmt.Fprintf(os.Stderr, "\tforgo doc <sym>[.<methodOrField>]\n")
	fmt.Fprintf(os.Stderr, "\tforgo doc [<pkg>.]<sym>[.<methodOrField>]\n")
	fmt.Fprintf(os.Stderr, "\tforgo doc [<pkg>.][<sym>.]<methodOrField>\n")
	fmt.Fprintf(os.Stderr, "\tforgo doc <pkg> <sym>[.<methodOrField>]\n")
	fmt.Fprintf(os.Stderr, "For more information run\n")
	fmt.Fprintf(os.Stderr, "\tforgo help doc\n\n")
...
func objectPath(userPath string, pkg *Package, symbol, method string) (string, string, error) {
	...
	if path == "." {
		...
		// forgo list to get the import path.
...
func parseArgs(flagSet *flag.FlagSet, args []string) (pkg *build.Package, path, symbol string, more bool) {
	...
	if slash >= 0 {
		...
		// invokes 'forgo list' and returns the error text from the loaded package.
